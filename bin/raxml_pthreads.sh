...
# assume 20 ml trees times 500 mb per tree = 10000 mb (10gb) mem needed
# double just in case
# try requesting 10 nodes
# srun --pty -p interact --mem 20000 -n 10 -N 1 -t 0-06:00 /bin/bash

# load RAxML
module load centos6/RAxML-8.1.15_openmpi-1.6.4_gcc-4.8.0

# compute 20 ML trees, save the best one
raxmlHPC-PTHREADS -T 10 -m GTRGAMMA -p 12345 -#20 -s ../data/rbcL_broad.phy -n rbcL_broad_20ML

# run 100 bootstraps
raxmlHPC-PTHREADS -T 10 -m GTRGAMMA -p 12345 -b 12345 -# 100 -s ../data/rbcL_broad.phy -n rbcL_broad_100boot
...
raxmlHPC-PTHREADS -T 10 -m GTRCAT -p 12345 -f b -t RAxML_bestTree.rbcL_broad_20ML -z RAxML_bootstrap.rbcL_broad_100boot -n rbcL_broad_MLboot 
